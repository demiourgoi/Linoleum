...
Linoleum processes a stream of OTEL spans, converting each span into a pair of __`SpanStart` and `SpanEnd` events__. 
__Events__ are __ordered__ by their epoch: start events use the span start as epoch, while end events use the end.
...
- `MaudeMonitorProperty`: based on a Maude program. 
  - We start from a specified Maude soup term, that should contain an object of a __monitor class__ that can be reduced to a truth value with `|=` as defined in Maude's `SATISFACTION` module. 
  - For each Linoleum event we create a message and add it to the soup, and __rewrite__ it according to the Maude program.
  - The final truth value is the result of __reducing__ the final soup using `|=` and specified Maude `Prop` term. If the resulting value is does not have a sort (Maude's `[Bool]` type for "error terms") then we assugn it the `Undecided` value.  
